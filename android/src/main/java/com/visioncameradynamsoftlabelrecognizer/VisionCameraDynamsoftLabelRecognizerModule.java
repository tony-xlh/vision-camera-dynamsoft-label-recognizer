...
public class VisionCameraDynamsoftLabelRecognizerModule extends ReactContextBaseJavaModule {
    ...
    public static CaptureVisionRouter router = null;
    ...
    @ReactMethod
    public void decodeBase64(String base64, String template, Promise promise) {
        WritableNativeMap scanResult = new WritableNativeMap();
        WritableNativeArray array = new WritableNativeArray();
        Bitmap bitmap = Utils.base642Bitmap(base64);
        try {
            CapturedResult result = router.capture(bitmap,template);
            RecognizedTextLinesResult linesResult = result.getRecognizedTextLinesResult();
            if (linesResult != null) {
                array.pushMap(Utils.getMapFromLinesResult(linesResult));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
  ...
  @ReactMethod
  public void initRuntimeSettings(String template, Promise promise) {
    try {
      router.initSettings(template);
      promise.resolve(true);
    } catch (CaptureVisionRouterException e) {
      promise.reject(String.valueOf(e.getErrorCode()),e.getMessage());
    }
    ...
    private void initLicense(String license, Promise promise){
      LicenseManager.initLicense(license, mContext, (isSuccess, error) -> {
        if(!isSuccess){
    ...
    private void initCVR() throws CaptureVisionRouterException {
        router = new CaptureVisionRouter(mContext);
        String template = """
          {
